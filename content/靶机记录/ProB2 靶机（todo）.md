...
我去访问：
![](assets/ProB2%20靶机（todo）/file-20260507174039984.png)
然后注意到了报错中说有一个什么`$data`参数，我就又开始乱试
吧`data`放到get/post都试一试 id、phpinfo()等都试过了，永远都是一样的回显。
但是发现一处不寻常的。
那就是只要传`?data=etc/passwd`，就回显403，而且像是正则匹配了，有其他字符也不影响，改一点点就又是原来的了，这就很奇怪了。
![](assets/ProB2%20靶机（todo）/file-20260507174039982.png)
那是不是说明`data`应该就是填路径了