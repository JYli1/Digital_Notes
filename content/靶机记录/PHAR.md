...
2. 然后我们注意到他是在`__get`魔术方法里（调用的成员属性不存在是自动执行），而且需要`Files`类的chen所以我们去看哪里可以触发这个条件。
...
```
这里我们完全控制调用的成员属性，只要把`level`赋值一个不存在的成员属性就好了，然后继续找谁可以调用`__toString`（把对象当作字符串调用时触发），
