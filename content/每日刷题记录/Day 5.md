...
```
这里的考点是 代码审计+`escapeshellarg 、escapeshellcmd连用造成的安全问题`
首先我们了解一下这两个函数
`escapeshellarg($host):

- **作用**：把字符串转义为合法的 Shell 参数。
    
- **原理**：它会在字符串的两侧加上**单引号**，并转义已有的单引号。
    
- **例子**：
    
    - 输入：127.0.0.1 -> 变为 '127.0.0.1'
        
    - 输入：a'b -> 变为 'a'\\''b'
`escapeshellcmd($host):

- **作用**：转义 Shell 中的特殊字符（如 #, &, ;, |, \*, ?, ~, <, >, ^, (, ), \[, ], {, }, $, \\, \x0A, \xFF 以及`不配对的引号`）。
    
- **原理**：在这些字符前加上反斜杠 `\`。
这里两个函数分开使用都没问题，但是两个一起使用，两次转义的情况下就会发生安全漏洞了
比如我们输入
```php
...
'\\'   <?=eval($_POST[1]);?> -oG shell.php \\''
```
大概就经历了这样一个过程吧，最后变成正常的参数了，（其实还一点没搞懂，再研究一下吧）
