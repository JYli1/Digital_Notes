...
# shrio550
RememberMe 功能开启后会在cookie中携带 RememberMe 字段，其中是我们的信息经过 序列化--> base64加密的字符串。
服务端收到后会 aes密钥解密+base64解密 -->反序列化 然后做校验，但是问题出在shrio550采用硬编码aes密钥。
所以我们可以用改密钥 对我们构造的恶意序列化字节流继续base64加密+aes密钥加密，然后替换RememberMe字段。造成java反序列化攻击，当然要搭配其他链子，如cc连