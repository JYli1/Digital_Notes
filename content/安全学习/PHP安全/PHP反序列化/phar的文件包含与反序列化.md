...
这里phar伪协议识别phar文件只要就是靠的stub中的`__HALT_COMPILER()`部分，所以上传文件时我们可以随意修改文件后缀。

还有添加十六进制文件头

做法就是**在`__HALT_COMPILER()`前面加，这前面加什么都无所谓

![[file-20251216153933020.png]]

当我们执行这个脚本后（区别就是stub里面加了一个头）

可以看到右边`<?php`前面有一个十六进制头，我们用file命令看一下这个文件

![[file-20251216153933013.png]]

看到系统认为他是一个GIF文件，这样可以绕过一些过滤。

## 5.3 利用方式

![[file-20251216153933023.png]]
